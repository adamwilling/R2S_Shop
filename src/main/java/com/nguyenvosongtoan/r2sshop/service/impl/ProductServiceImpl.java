...
public class ProductServiceImpl implements ProductService {
	...
	public ProductServiceImpl(ProductRepository productRepository, VariantProductRepository variantProductRepository,
			ProductMapper productMapper, VariantProductMapper variantProductMapper, CategoryService categoryService) {
	 ...
	 * @return Đối tượng PaginationDTO chứa danh sách sản phẩm và thông tin phân
	 *         trang
	 * @throws Exception Nếu không tìm thấy sản phẩm
	 */
	@Transactional(readOnly = true)
	@Override
	public PaginationDTO findAllProductByCategoryId(long categoryId, int no, int limit)
			throws ProductNotFoundException {
		Page<ProductCreateDTO> productCreateDTOPages = productRepository
				.findAllByCategoryIdOrderByPriceDesc(categoryId, PageRequest.of(no, limit))
		...
		return new PaginationDTO(productCreateDTOPages.getContent(), productCreateDTOPages.isFirst(),
				productCreateDTOPages.isLast(), productCreateDTOPages.getTotalPages(),
				productCreateDTOPages.getTotalElements(), productCreateDTOPages.getSize(),
				productCreateDTOPages.getNumber());
	}

	/**
	 * Tạo mới một sản phẩm.
	 *
	 * @param productCreateDTO Đối tượng ProductCreateDTO chứa thông tin sản phẩm
	 *                         cần tạo
	 ...
	 *
	 * @param updateProductDTO Đối tượng UpdateProductDTO chứa thông tin sản phẩm
	 *                         cần cập nhật
