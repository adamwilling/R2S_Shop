...
public class CartLineItemServiceImpl implements CartLineItemService {
    ...
    public CartLineItemServiceImpl(CartLineItemRepository cartLineItemRepository, CartLineItemMapper cartLineItemMapper, UserService userService, CartService cartService, ProductRepository productRepository, CartMapper cartMapper, UserMapper userMapper, CartRepository cartRepository) {
    ...
    public List<CartLineItemDTO> getAllByCartId(long id) {
        return cartLineItemRepository.findAllByCartId(id).stream().map(cartLineItem -> cartLineItemMapper.toDTO(cartLineItem)).toList();
    }
     ...
     * @throws Exception Ném ngoại lệ nếu có lỗi xảy ra trong quá trình thêm vào giỏ hàng
     */
    @Transactional
    @Override
    public CartLineItemDTO addProductIntoCart(AddCartLineItemDTO addCartLineItemDTO) throws Exception {
        UserDTO user = userService.getCurrentUser();
        Product product = productRepository.findById(addCartLineItemDTO.getProductDTO().getId()).orElseThrow(() -> new Exception("Sản phẩm không tồn tại!"));
        CartLineItem cartLineItem = cartLineItemMapper.toEntity(addCartLineItemDTO);
        cartLineItem.setProduct(product);
        List<Cart> carts = cartService.findByUserIdAndStatus("new").stream().map(cartDTO -> cartMapper.toEntity(cartDTO)).toList();
        if (carts.isEmpty()) {
            ...
            cartLineItem.setCart(cart);
            Optional<CartLineItem> existedCartLineItem = cartLineItemRepository.findByCartIdAndProductId(cart.getId(), product.getId());
